...
class NewAccount: UIViewController ,UIPickerViewDataSource, UIPickerViewDelegate, UITextFieldDelegate , UITextViewDelegate {
                        ...
                        else
                        {
                            ref.authUser(self.EmailAddressTF.text, password: self.PasswordTF.text, withCompletionBlock: { (error, authData) -> Void in
                                ...
                                else
                                {
                                    ...
                                    /*
                                    //addition:
                                    self.ref.childByAppendingPath("Categories").childByAppendingPath(self.itemSelected).childByAppendingPath("\(authData.uid)").setValue("True")
                                    */
                                }
                            })
    ...
    func textViewDidBeginEditing(textView: UITextView) {
        if textView == DetailsTV{
            NewAccountSV.setContentOffset(CGPointMake(0, 700), animated: true)
        }
    }
    ...
    func textFieldDidBeginEditing(textField: UITextField) {
        ...
        if textField == BusinessNameTF{
            NewAccountSV.setContentOffset(CGPointMake(0, -20), animated: true)
        }
        if textField == ShortDescriptionTF{
            NewAccountSV.setContentOffset(CGPointMake(0, 60), animated: true)
        }
        if textField == CityTF{
            NewAccountSV.setContentOffset(CGPointMake(0, 300), animated: true)
        }
        if textField == ContactMeTF{
            NewAccountSV.setContentOffset(CGPointMake(0, 385), animated: true)
        }
        if textField == PhoneNumberTF{
            NewAccountSV.setContentOffset(CGPointMake(0, 480), animated: true)
        }
        if textField == Website1TF{
            NewAccountSV.setContentOffset(CGPointMake(0, 600), animated: true)
        }

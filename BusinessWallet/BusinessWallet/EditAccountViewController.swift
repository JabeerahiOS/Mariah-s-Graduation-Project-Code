...
class EditAccountViewController: UIViewController,UIPickerViewDataSource, UIPickerViewDelegate, UITextFieldDelegate, UITextViewDelegate {
    ...
    var CategoryArray:[String]=["Cooking","Beauty","Student Services","Art & Design","Store", "Others"]
            ...
            else
            {
                ...
                ref.changeEmailForUser(mainEmail, password: mainpass, toNewEmail: self.EmailAddressTF.text, withCompletionBlock: { error in
                    ...
                    if error != nil {
                        ...
                        self.ref.childByAppendingPath("Users").childByAppendingPath(self.ref.authData.uid).updateChildValues(["Email":self.EmailAddressTF.text!])
                ...
                ref.changePasswordForUser( NSUserDefaults.standardUserDefaults().objectForKey("Email") as? String, fromOld: NSUserDefaults.standardUserDefaults().objectForKey("password") as? String, toNew: self.PasswordTF.text)
                    {
                        ...
                        if ErrorType != nil {
                            ...
                            if let errorCode = FAuthenticationError(rawValue: ErrorType.code) {
                        ...
                        else
                        {
                            ...
                            self.ref.childByAppendingPath("Users").childByAppendingPath(self.ref.authData.uid).updateChildValues(["Provider":self.PasswordTF.text!])
                            self.navigationController?.popViewControllerAnimated(true)
                        }
    ...
    func throwAlert()
    {
        ...
        let alert = UIAlertController(title: "Oops!", message:"Network Error", preferredStyle: .Alert)
        alert.addAction(UIAlertAction(title: "OK", style: .Default) { _ in })
    ...
    func textViewDidBeginEditing(textView: UITextView) {
        if textView == DetailsTV{
            EditAccountSV.setContentOffset(CGPointMake(0, 850), animated: true)
        }
    }
    ...
    func textFieldDidBeginEditing(textField: UITextField) {
        ...
        if textField == BusinessNameTF{
            EditAccountSV.setContentOffset(CGPointMake(0, 15), animated: true)
        }
        if textField == ShortDescriptionTF{
            EditAccountSV.setContentOffset(CGPointMake(0, 105), animated: true)
        }
        if textField == CityTF{
            EditAccountSV.setContentOffset(CGPointMake(0, 380), animated: true)
        }
        if textField == ContactMeTF{
            EditAccountSV.setContentOffset(CGPointMake(0, 460), animated: true)
        }
        if textField == PhoneNumberTF{
            EditAccountSV.setContentOffset(CGPointMake(0, 550), animated: true)
        }
        if textField == Website1TF{
            EditAccountSV.setContentOffset(CGPointMake(0, 675), animated: true)
        }
